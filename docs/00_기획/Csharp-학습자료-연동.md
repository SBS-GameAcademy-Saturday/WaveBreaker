...
| 파일 | 무엇 |
|---|---|
| [`부록A-CSharp-더-알아야-하는-것.pptx`](../04_배포자료/슬라이드/부록A-CSharp-더-알아야-하는-것.pptx) | 위 4장을 **심화 덱**으로 — 성능(힙·GC·**풀링**·문자열·박싱) · 구조 · 안전 |

> 🔑 **덱은 이 문서의 4장보다 넓다.** 이 표는 "저장소의 어느 파일을 언제 쓰나" 이고,
> 덱은 "게임을 만들려면 문법 밖에 무엇이 더 필요한가" 다. 풀링이 그 덱의 중심(16장 중 4장)이다.
> 회차 번호가 바뀌면 양쪽을 같이 고친다.
