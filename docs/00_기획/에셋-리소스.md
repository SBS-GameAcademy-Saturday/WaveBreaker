...
| `Tiny RPG … 01 / 02` | Sprite | Point | None | **100** | Multiple · 100×100 Grid |
...
> 기존 32px 스프라이트(PPU 32)와 크기가 자동으로 맞는다. Soldier 100×100 을 PPU 32 로 넣으면 3배로 커진다.
...
### 8.2-b 🔴 남은 문제 — 캐릭터가 기존 것보다 4배 작다

임포트는 고쳤지만 **보이는 크기가 안 맞는다.** 칸 안에서 그림이 실제로 차지하는 픽셀을 재봤다.
...
| 안 | 무엇 | 결과 |
|---|---|---|
| **A** | Tiny RPG PPU **100 → 25** | 21px ÷ 25 = **0.84** — 기존 0.81 과 맞는다. 스케일 손 안 댄다 |
| B | PPU 100 유지 | 카메라 `orthographicSize` 를 5 → 약 1.2 로. **기존 스프라이트·타일·UI 전부 다시 맞춰야 한다** |
| C | 기존 placeholder 를 새 아트 기준으로 교체 | 어차피 자리 표시였으니 새 아트가 기준이 된다. 콜라이더·속도 재조정 필요 |

> ⚠️ **아직 안 골랐다.** A 가 손이 가장 덜 가지만, 어느 쪽이든 **아트 방향 결정**이라 임의로 바꾸지 않았다.
...
>
> **대응 (택 1)**
> 1. 학생이 **각자 itch.io 에서 직접 내려받게** 하고, 수업은 폴더 배치만 안내한다 — 가장 안전하다
> 2. 재배포가 허용되는 팩(CC0 등)으로 교체한다
> 3. 제작자에게 교육용 재배포 허가를 문의한다
>
> 🔴 **6절의 학생 배포용 문구도 그대로 쓰면 안 된다.** "전부 CC0" 가 아니다.
> **127회차 개인 미션 ②의 답(그래픽·사운드 = CC0)** 과 **Phase 10 위험 신호("에셋이 CC0라 …")** 도 같이 고쳐야 한다.
...
- [ ] 🔴 **캐릭터 크기 결정** (8.2-b 의 A/B/C) — 지금은 기존 것의 1/4 로 보인다
- [ ] FX 팩은 쓸 이펙트를 고른 뒤 그 파일만 64×64 슬라이스
- [ ] 나머지 4종 라이선스 확인 후 8.3 표 채우기
- [ ] 재배포 가능 여부 확정 → 배포 방식 결정 (직접 내려받기 vs 통합 패키지)
- [ ] 6절 · 127회차 · Phase 10 의 "CC0" 표현 수정
- [ ] 어느 캐릭터를 Player / 몬스터 3종에 배정할지 결정 (Soldier · Orc · Demon_A · Blood Monster_A)
