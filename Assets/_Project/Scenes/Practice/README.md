...
## 24주차 (116–120) — 세 번의 회수 · 서버 권한

> 🔴 **이번 주에 처음으로 본 게임 스크립트를 건드렸다.** 22주차에 "24주차부터" 라고 예고한 대로다.
> 다만 **씬은 안 건드렸고**, 스크립트도 싱글에서 동작이 안 바뀌는 것만 넣었다.

### 새 파일

| 파일 | 하는 일 |
|---|---|
| `Scripts/Network/NetworkRole.cs` | **`IsServerOrOffline`** — 싱글을 "호스트 1인" 으로 취급 |
| `Scripts/Network/NetworkWaveManager.cs` | 서버만 스폰 · `Instantiate` + `NetworkObject.Spawn()` |
| `Scripts/Network/NetworkEnemy.cs` | 서버가 움직이고 판정 · 죽으면 젬 Spawn |
| `Scripts/Network/NetworkGem.cs` | 서버 단독 판정 (`IsServer` + `taken`) |
| `Scripts/Network/NetworkTeam.cs` | 팀 경험치·레벨 (기획서 11장) |
| `Scripts/Network/NetworkPlayerAttack.cs` | `NetworkObjectId` 를 Rpc 로 보내 서버가 판정 |
| `Prefabs/Network/NetworkEnemy.prefab` `NetworkGem.prefab` | Authority = **Server** |

### 본 게임에서 바꾼 것 — 스크립트 3개, 씬 0개

| 파일 | 무엇 | 싱글 영향 |
|---|---|---|
| `WaveManager.cs` | `SpawnOne` / `SpawnBoss` 에 `if (!NetworkRole.IsServerOrOffline) return;` | **없음** (항상 true) |
| `CameraFollow.cs` | `AddTarget` / `RemoveTarget` 추가 | **없음** (계산 코드 그대로) |
| `NetworkPlayerTag.cs` | 카메라 목록에 자기 등록 | 연습 씬 전용 |

> 🔑 **싱글은 "호스트 혼자 하는 게임" 으로 취급한다.**
> `NetworkManager` 가 없거나 접속 안 했으면 `IsServerOrOffline` 이 true 다.
> 그래서 본 게임 코드에 `if (협동) ... else ...` 분기가 **하나도 안 생긴다.**

> ✅ **자동 검증 완료 (2026-09-03)**
>
> **협동 — 2인 접속** (`Player 2` 태그 `Client` 자동 접속)
>
> ```
> IsHost=True  접속자=2명  목록=[0, 1]
> 몬스터 1마리   젬 2개   플레이어 2명
> 네트워크 오브젝트 총 7개          ← 플레이어2 + 몬스터1 + 젬2 + 매니저2
> 팀 — 경험치 6/8  레벨 2
>    플레이어 0  IsOwner=True   체력 20
>    플레이어 1  IsOwner=False  체력 20
>    몬스터 30  위치 (-4.9, -3.7)  서버소유=True
> ```
>
> 오브젝트 수가 딱 맞는다 — **몬스터가 두 배로 안 나왔다는 증거**다.
>
> **로그**
>
> ```
...
> ```
>
> 처치 로그가 **호스트에만** 찍힌다 (판정이 거기서 났으니까). `Rpc` → 서버 판정 → `Despawn` 의
> 전체 사슬이 돈다는 뜻이다.
>
> **협동 카메라 (120)**
>
...
> ```
>
> ⚠️ **클라이언트 쪽 화면 상태는 미실측.** 23주차와 같은 이유다.
> ⚠️ **키보드 이동은 미실측.** 대신 공격을 **자동 발사**로 만들어 몬스터 처치·젬 드롭까지는 실측했다.
> ⚠️ **Relay 인터넷 접속 미실측.** 124회차 내용이다.
> ⚠️ **네트워크 판 10분 완주·밸런스 미실측.**

### ★ 겪은 것 — 맞은 몬스터가 하얗게 남았다

`OnHealthChanged` 에서 흰색으로 바꾸기만 하고 **되돌리는 코드를 안 넣었다.**
스크린샷을 보고 발견했다 — 화면에 흰 네모가 하나 떠 있었다.

099회차의 `Flash` 와 같은 구조(코루틴 + 원래 색 복귀)로 고쳤고, 프리팹의 `bodyColor` 를
실제 스프라이트 색과 맞췄다. **119 강의안의 도전 미션**에 "양쪽 화면에서 번쩍이게 한다" 로 남겼다.

### ★ 겪은 것 — 재컴파일하면 가상 플레이어 접속이 끊긴다

색 버그를 고치고 다시 재려니 **접속자가 1명**으로 나왔다. `Player 2` 는 `Launched` 인데
클라이언트로 안 붙어 있었다. 도메인 리로드 후 자동 접속이 다시 안 걸린 것으로 보인다.

`Deactivate` → `Activate` 로 다시 띄우니 2명이 됐다. **수업 중에도 나올 수 있는 상황**이라
111회차 사고표의 "클라이언트 접속 실패 → 다시 Play" 항목이 여기에 해당한다.

### ★ 설계 판단 — 본 게임에 씬은 안 건드렸다

22주차에 "24주차부터 본 게임에 옮긴다" 고 예고했다. 그런데 **씬은 아직 안 건드렸다.**

`Enemy` `ExpGem` 을 `NetworkBehaviour` 로 바꾸면 `NetworkObject` 가 필요하고,
스폰 경로가 `PoolManager` 에서 `NetworkObject.Spawn()` 으로 바뀐다.
그건 **싱글 경로를 통째로 갈아엎는 일**이라 Phase 9 문서가 최대 사고로 지정한 것에 정면으로 걸린다.
...
## 26주차 (126–130) — 링크로 보낸다 · Phase 10 종료

> 🔴 **코드를 한 줄도 안 고쳤다.** 126회차부터 기능 동결이라 그게 맞다.
> 바뀐 것은 `ProjectSettings` 의 `bundleVersion` **한 줄**뿐이다.

### 바꾼 것

| 파일 | 무엇 |
|---|---|
| `ProjectSettings/ProjectSettings.asset` | `bundleVersion: 0.1.0` → **`1.0.0`** |
| (자동) `preloadedAssets` | 빌드가 TMP Settings 를 넣었다 |
| (자동) `Pretendard SDF.asset` | 빌드가 동적 아틀라스를 비웠다 — **정상** |

### 새 산출물 (전부 `Builds/`, `.gitignore` 대상)

| 파일 | 크기 |
|---|---|
| `Builds/WaveBreaker_Final/` | 108.8 MB · 245개 파일 |
| `Builds/WaveBreaker_v1.0.0_Windows.zip` | 41.3 MB |
| `Builds/WaveBreaker_Source_v1.0.0.zip` | 2.5 MB |

---

> ✅ **자동 검증 완료 (2026-09-03)**
>
> **126 — 최종 빌드 (`BuildPipeline.BuildPlayer`)**
>
> ```
> Version = 1.0.0
> 빌드에 들어갈 씬 4개
...
>    Assets/_Project/Scenes/Coop.unity
>
> 결과       = Succeeded
> 에러       = 0   경고 = 0
> 걸린 시간   = 49.6초
...
> ```
>
> **21주차와 비교** — 105.2 MB / 203개(3씬) → 108.8 MB / 245개(4씬).
> **협동 20회차 전부가 +3.6 MB, +42개 파일**(NGO · Relay 라이브러리)이다.
>
> **126 — 빌드 실행**
>
> ```
...
> ```
>
> 로그는 `AppData\LocalLow\WaveBreaker Class\웨이브 브레이커\Player.log` — **38줄, 예외 0개**.
> `Exception`/`NullReference` 검색 0건. 유일하게 걸리는 문자열은
> `d3d12: failed to query info queue interface (0x80004002)` 인데 **에러가 아니다**(디버그 레이어 없음).
>
> **126 — 빌드 대상 지원 여부**
>
> ```
...
> ```
>
> **126 — 한글 경로 빌드 (104회차 경고 재검증)**
>
> ```
> 출력 경로 = Builds\한글 경로 테스트
> 결과     = Succeeded   에러 = 0   경고 = 0
> 걸린 시간 = 11.0초
> 실행     = 살아있음, 604 MB, 창 제목 "웨이브 브레이커"
> ```
>
> **127 — 배포용 zip**
>
...
> 압축 시간 =   8.8초
> itch.io 무료 한도 1 GB 이내 = True
> ```
>
> **130 — 원본 zip**
>
> ```
> Assets + ProjectSettings + Packages =    10.2 MB  →  zip  2.5 MB (7.6초)
> Library                             = 3,206.7 MB
> Temp                                =    81.8 MB
> docs                                =    18.9 MB
> ```
>
> Library 를 넣으면 **1310배**가 된다.
>
> **스크린샷 (실측)**
>
> | 파일 | 무엇 |
> |---|---|
> | `126_Build_Title.png` | **빌드된 게임**의 타이틀 화면. 1280×720 |
> | `128_Gameplay.png` | 에디터 Game View. 00:14 · 처치 5 · Lv.1 · 체력 20/20 |
>
> ⚠️ **빌드 안에서 버튼을 눌러 씬을 옮기는 것은 미실측.** 빌드에 마우스·키보드를 넣지 않았다.
> 씬 4개가 빌드에 들어간 것과 프로세스·로그가 정상인 것까지만 쟀다.
> ⚠️ **itch.io 업로드와 링크 발급은 미실측.** 계정 생성과 외부 게시가 필요해 하지 않았다.
> 대신 **올릴 파일(zip 41.3 MB)** 은 실측했다.
> ⚠️ **시연 영상·GIF 녹화는 미실측.** 수업 중 Win+G 로 진행하는 항목이다.
> ⚠️ **WebGL 빌드는 미실측** — 모듈이 없다.

### ★ 겪은 것 — 빌드된 게임을 안전하게 찍는 법을 찾았다

21주차에는 빌드 화면이 **미실측**이었다. 그때 PowerShell 로 "앞에 있는 창" 을 캡처했다가
**게임이 아닌 다른 창**이 찍혔고, 재시도하지 않고 미실측으로 남겼다.

이번에는 `Process.MainWindowHandle` 로 **그 프로세스의 창만** `PrintWindow` 했다.
z-order 와 무관하게 대상 창만 찍히므로 다른 창이 끼어들 수 없다.

```
HWND = 7933304   제목 = 웨이브 브레이커
캡처 크기 = 1280 x 720
```

### ★ 겪은 것 — 104회차의 경고는 절반만 맞았다

104회차 강의안이 **"경로에 한글이 하나라도 있으면 빌드가 실패합니다"** 라고 못박았다.

| 무엇의 경로인가 | 결과 |
|---|---|
| **빌드 출력 폴더** | **된다.** `Succeeded` · 에러 0 · 11.0초 · 실행까지 확인 |
| **프로젝트 폴더** | **미실측.** 3.2 GB 프로젝트를 한글 경로로 옮겨야 해서 안 했다 |

104회차가 말한 건 **프로젝트 경로**였다. 학생이 `바탕화면\내 게임 빌드` 처럼
**출력 폴더**를 한글로 잡는 건 문제가 없다. 126 강의안 도전 과제에 이 구분을 그대로 적었다.

### ★ 겪은 것 — 빌드하면 폰트 에셋이 매번 바뀐다

`Pretendard SDF.asset` 이 3221줄 추가 / 2677줄 삭제로 변경된 것으로 떴다. 원인은 이거였다.

```
TMP Settings → Clear Dynamic Data On Build = True
아틀라스 : 1024 x 1024, 1장
담긴 글자 : 96개 (Play 후)  →  55개 (빌드 후)
에셋 파일 : 2512 KB
```

**Dynamic 폰트는 필요한 글자만 아틀라스에 그려 넣는다**(093회차). 에디터에서 돌릴수록 글자가 쌓이고,
TMP 는 빌드할 때 그걸 **다시 비운다**. **버그가 아니다.** 126 강의안 사고표에 넣었다.

### ★ 설계 판단 — Phase 10 회차 배분을 바꿨다

Phase 10 문서는 원래 `129–130 최종 발표회` 로 **두 회차를 통째로** 발표에 잡아뒀다.

그런데 129 강의안을 쓰다 보니 **리허설이 없으면 5분이 시연 준비로 다 날아간다.**
"발표 사고의 90%가 시연에서 난다" 는 게 이 Phase 의 실질적 위험이고, 그건 연습으로만 줄어든다.

그래서 **129를 준비·리허설로, 130을 발표회로** 나눴다. Phase 10 문서에 이유와 함께 반영했고,
인원이 많으면 129 후반을 발표 1부로 쓰라는 단서를 달았다.
